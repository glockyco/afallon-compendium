...
const complete: AtlasState = {
  mapSpaceId: "coalway", layerIds: ["game-coalway"], selectedPlacementId: "placement",
...
test("ignores removed aliases and never mutates prior state", () => {
  const parsed = readAtlasUrl("?layer=old&roles=enemy&level-min=1&level-max=2");
  expect(parsed.layerIds).toEqual([]);
  expect(parsed.categories).toEqual(DEFAULT_ATLAS_STATE.categories);
  const next = transitionAtlasState(parsed, { type: "select-map", mapSpaceId: "world", layerIds: ["game-world"] });
  expect(parsed.mapSpaceId).toBeNull();
  expect(next).toMatchObject({ mapSpaceId: "world", layerIds: ["game-world"], selectedPlacementId: null });
  expect(Object.isFrozen(next)).toBe(true);
  expect(writeAtlasUrl(new URL("https://atlas.test/?layer=old"), next).searchParams.has("layer")).toBe(false);
});
