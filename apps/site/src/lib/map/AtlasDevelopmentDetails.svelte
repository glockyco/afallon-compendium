...
<aside class="details-panel" bind:this={detailsPanel} aria-label="Development selection details">
  ...
  {#if document}<FactCard {document} {registry} compact showRelations limit={5} /><details class="entity-block"><summary>Published document</summary><pre>{JSON.stringify(document, null, 2)}</pre></details>{/if}
