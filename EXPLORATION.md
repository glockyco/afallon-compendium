...
Use `withRuntime` for every extraction, traversal, and capture operation. `Runtime.evaluate` takes a C# expression; `Runtime.probe` wraps a reviewed probe body. Both expose `registerRuntimeCleanup` and `registerFrameCleanup` to that C# code. Register cleanup before allocating resources or changing state. Runtime cleanup retains resources across frames and returns an unregister action; unregister only after explicit cleanup. Frame cleanup runs in the evaluation's `finally` block, including on exceptions. Callbacks run in reverse registration order and must finish synchronously. An evaluation error ends the owner; subsequent checks need a new owned operation.
