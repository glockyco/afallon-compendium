...
`packages/contracts/src/public/static.test.ts` gains schema fixtures per kind. `publication-parity.ts` refuses a candidate that drops published pages or artwork. The deploy smoke test requests one page per kind, one list, the coverage page, and one redirect. `PUBLICATION_SCHEMA_VERSION` moves to `compendium.publication.v14`, and the tracked baseline is regenerated by publishing once with the new pipeline.
