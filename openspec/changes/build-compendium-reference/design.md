...
Guide contracts (`PublicAdventureGuide*`, `GuideDocument`, `StaticGuideDocument`), `guide-projection.ts`, `guide-resources.ts`, `GuideBrowser.svelte`, `GuideBossDetails.svelte`, `guide-load.ts`, and the `/guide` routes are removed outright. There is no redirect file, no `?id=` resolver, and no key-to-path resource: a redirect layer would keep the old vocabulary alive in URLs, in the site, and in the publication for the sake of links to an early-access page whose content now lives under its own kind. An old link reaches the not-found page, which already offers the map and search. Task 6a.10 in `build-screenshot-first-map` is marked superseded by this change.

### 13. One shape at a time, no transition period
...
It verifies a baseline publication root through the same graph verification as the candidate and
compares the verified resources, so the baseline cannot be a legacy artifact in an older shape.

### 14. Page layout

The first pages rendered every fact in one full-width slab of hairline label-and-value rows, which
buries the facts that matter. Ancient Kingdoms is the reference for the shape, and it is public, so
compare against it directly rather than describing it:
...
- Facts as label-above-value pairs in a two or three column grid, not label-left and value-right
  across a wide row.
- Stats and resistances as a compact chip grid with the name left and the signed value right.
- Tables with zebra rows, right-aligned numbers, sortable headers, and an icon before each name.
- Colour that carries meaning and nothing else: rarity on an item's name and icon ring, currency on
  prices, one accent for links and focus. The rest stays neutral.
- Filters as labelled controls in a single row, not raw multi-select boxes.

The game's own item tooltip is the second reference, captured in
`research/screenshots/reference/game-item-tooltip-weapon.png` and `-armor.png`. It leads with the
icon and the name in its rarity colour, then the slot and the type on one line, then `Item Power`,
then the damage range with a derived damage per second, then stats as `+21 Stamina` and
...
`packages/contracts/src/public/static.test.ts` gains schema fixtures per kind. `publication-parity.ts` refuses a candidate that drops published pages or artwork. The deploy smoke test requests one page per kind, one list, the coverage page, and the not-found page. `PUBLICATION_SCHEMA_VERSION` moves to `compendium.publication.v14`, and the tracked baseline is regenerated by publishing once with the new pipeline.
...
2. Land the site routes and panel and delete the guide tree; stage the candidate; run parity, graph verification, and the preview.
3. Deploy once. The `/guide` paths stop existing; the Steam guide and project page links to them need updating, which is cheaper than carrying a redirect layer.
