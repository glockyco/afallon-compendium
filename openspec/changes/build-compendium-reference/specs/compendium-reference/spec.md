...
### Requirement: Guide routes redirect to their pages

Requests to `/guide/` and its former section routes SHALL redirect to the corresponding place, NPC, or property page or list. The former dungeon, boss, region, and property content SHALL remain reachable through those pages.

#### Scenario: A reader follows an old boss link
- **WHEN** the reader requests `/guide/bosses/?id=<key>`
- **THEN** the site redirects to that NPC's page
