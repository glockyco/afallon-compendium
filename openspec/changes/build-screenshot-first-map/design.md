...
An illustration is an ordinary layer drawn beneath the captured tiles in world coordinates, with the markers and the camera unchanged. The published image is bounded to a 4096-pixel edge and encoded lossy, with a reviewed transform scaled with it, because the 7540 by 8192 source decodes to 236 MB of RGBA and wedged the renderer before it drew. One illustration without a reviewed transform is fitted to its map's world bounds and labelled approximate on screen.
