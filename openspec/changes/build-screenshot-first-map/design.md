...
### 1. One repository with one-way workspace boundaries

The implemented layout uses `apps/compendium-cli` for command composition, `apps/site` for the static interface, and one-way workspace packages for contracts, artifacts, runtime access, scanning, capture, catalog assembly, and publication. `tools/`, `pipeline/`, and the root `site/` are obsolete and absent.

Use TypeScript with Bun for host commands and catalog assembly, SQLite as the only normalized source of truth, and SvelteKit with static output for the site. Reuse the existing HotRepl client protocol/SDK instead of creating another runtime server. Publication queries SQLite directly and emits content-addressed static resources without copied run inputs or normalized JSON projections.
...
The operator commands are `scan`, `capture`, `catalog`, `publish`, `preview`, and `deploy`. `scan` replaces separate extraction and traversal commands. `catalog` replaces normalization as the public command, and `publish` replaces separate tile and publication commands. Capture accepts explicit camera plans and visits requested source scenes under runtime ownership. A sweep claims ownership once, restores frame-local state and streams per plan, and finishes in the configured known-good scene, Coalway woods, rather than returning to the scene active at the start. This avoids a save that cannot load out of the corrupted challenge-stone arena (scene 16). Capture requires a reviewed map-space profile and stable tile-local geometry before rendering but does not establish complete imagery coverage. Catalog assembly and publication consume verified artifacts without connecting to the game. Configuration supplies explicit game paths, HotRepl endpoint, research character, final scene, and output locations. The installed game supplies the build identity.

A scan includes canonical records, relationships, loot rules, world inventory, NPC producers, world sources, faction rules, and placement snapshots for explicit current-scene, build-scene, or streamed-source targets. It validates schemas, counts, and references. Each observation records scene and character context. Sequential runtime calls are not one simultaneous observation. A successful bounded scan does not establish full-world coverage.

Two instrumented games cannot share one HotRepl port, and the connection reaches whichever bound first. `HOTREPL_PORT` gives each game its own port, and Afallon uses 18601. Runtime acquisition verifies the connected product before it claims ownership, so a wrong game fails `scan` or `capture` instead of producing evidence from another title.
...
Content evidence alone frames a map badly. Coalway catacombs holds 109 placements in a 300-unit cluster, but a few scene-independent objects at fixed coordinates such as (1490, 45, 2101) and (-49, -1845, -4208) recur in most scenes and stretched its frame to 768 by 768 with the content in one corner. A reviewed box domain is therefore the frame of every interior map. `author-domain-boxes.ts` proposes each box from the MapZone rectangle grown toward scene-local placements within 96 units, treating a position that recurs in three or more scenes as scene-independent, and snaps it to a 128-unit lattice so capture tiles fit without padding. The reviewer fine-tunes the coordinates in the profile. Normalization records placements outside the boxes in an exclusion ledger with binding evidence, and publication reports them instead of counting them as omitted coverage.

Afallon marks foliage with no Unity layer or tag: trees, sequoias, and impostor billboards sit on layer 9 `Ground` with cliffs and rocks, and only one camera exists (survey run 28a00887). The shader family is the only stable marker: `Custom/VegetationLeaves`, `Custom/VegetationTrunk`, `Polyart/Dreamscape Foliage`, `Polyart/Dreamscape/Builtin/Tree Wind`, `BK/Impostor`, and `Hidden/Amplify Impostors/`. Terrain-instanced trees hide through `Terrain.drawTreesAndFoliage`. The capture plan carries that reviewed list, capture hides and restores the matching renderers with the same audit as other suppressed visuals, and geometry readiness excludes them.

The overworld owner rule assigns each lattice cell to one scene, so a stale plan is a correctness problem rather than clutter: `capture-world-surface-15.json` survived a planner run that no longer gave scene 15 any cell, and its three cells were also owned by scenes 3 and 9. Two owners rendering one cell made the pyramid reject itself with contradictory pixels at `(1024, -1)`. The planner therefore deletes the plan of every scene that owns no tile and reports what it removed. Retired experiment plans do the same damage through a different route: `capture-coalway-v5`, `capture-coalway-baseline`, `capture-coalway-transitions`, `capture-mine-v5`, and `capture-coverage` kept resurrecting a `coalway` map space on a retired calibration, so they are removed.
...
The overworld's reviewed box is the playable region the game draws as a red outline in its own map texture. With the artwork calibrated, that outline's bounding box converts to world coordinates: x from -975 to 1934 and z from -2457 to -23, grown by 128 units and snapped to the 128-unit lattice as x from -1152 to 2176 and z from -2688 to 128. Every world surface binding carries that box, so the capture planner clips the terrain rectangle to it, normalization excludes placements outside it with reasons, and the artwork is clipped to it. The overworld capture went from 82 tiles, most of them terrain nobody can reach, to 40.
...
Each probe source compiles once per runtime session into a delegate the runtime owner holds, keyed by the hash of prelude, body, and observation-context mode; every later call invokes that delegate with fresh arguments, output path, and cleanup registrars. Compiling per call had made each evaluation a new script assembly the game never unloads: one Duskfall tile cost ten probes, so a zone added over two hundred assemblies, tile time grew from 28 to 41 seconds across one run, and the game reached 34 GB. Measured after the change: a cached geometry probe takes 0.5 seconds instead of 6, one tile takes 7 seconds instead of 28 to 41, and the game holds under 8 GB through a zone. The scene-boundary memory reclamation that fought the growth is removed with its cause; it was the step that timed out and failed every sweep.
...
Capture writes the tile-checkpoint contract defined in `packages/contracts/src/capture/evidence.ts` after tile rendering, registration, visual restoration, and stream cleanup pass. Compatibility hashes include build, profile, implementation, character, scene, cut survey, suppression, resolution, lighting, readiness, and frame. Unrelated tiles do not change a compatible tile's key.
...
`CaptureSet` in `packages/contracts/src/capture/evidence.ts` binds the exact expected tile list to validated checkpoints before atomic successful-run selection. A failed run cannot replace that selection. Capture sets do not establish full-world imagery. The publication gate makes that separate decision.
...
Pin deck.gl `9.3.7` and its Luma WebGL stack to `9.3.3`, matching the proven Ancient Kingdoms renderer generation. Deck.gl `9.4.0` changed GPU attribute buffering, render passes, and line antialiasing; the user observed corrupted lines and a full-canvas cyan primitive on Windows 10 with the Firefox-based Zen Browser after that upgrade. Do not adopt the 9.4 renderer until the atlas passes hover and selection checks on that browser family.
...
Use explicit rings above markers for result previews and selection. The primary placement uses a larger gold ring. Other placements with the same exact entity key use white group rings. A result-preview ring renders above a group ring and below the primary ring. Entity and item result previews resolve through the existing placement indexes rather than matching labels. Map-pointer hover uses an isolated ring overlay and a global hover callback. Pointer movement replaces only that small non-pickable overlay; it does not reconstruct the picked marker layer or leave stale hover state. Configure deck.gl's click recognizer with a minimal interval so a single click does not wait for the default double-click window before selection. Double-click zoom remains enabled, and its first click also selects.

Travel connections follow the sibling atlas's zone-line arrangement: three coordinated layers under one visibility key, being a line from source to destination, a mark at the destination, and the travel marker itself, with disabled entries recolored rather than removed. Selection and hover strengthen the applicable line without hiding its disabled state. Keep that grouping. Connections also serve the world layout: with them visible in the authoring mode, a stretched or crossed line exposes a wrong offset immediately, which is cheaper than eyeballing terrain edges.

Afallon's current public contract has typed spatial endpoints only for travel transitions. It has no published trap teleport, patrol, summon-blocker, or similar endpoint pair. Do not infer another connection from labels or detail text. A future connection family requires an explicit typed source and destination in the public contract before the atlas can render it.
