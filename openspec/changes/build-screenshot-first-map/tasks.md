...
- [x] 4.5 Cut covered maps along the walkable surface. A plan cites its hashed navigation survey; each tile rasterizes the walkable height under every pixel, renders one slice per step across its range in one frame-local operation, and composes each pixel from the first slice above its surface plus headroom. Verified on Duskfall Depths: a tile spanning 170 units of walkable height shows floor, water, paths, and ledges with no ceiling. No scene object is disabled, deleted, or left modified.
...
- [x] 4.7 Implement resumable capture manifests and hashed image output. Verify changed build/profile inputs invalidate reuse and interrupted capture leaves the prior valid set intact. The cut derivation is a hashed input, so a tile captured under an earlier derivation is recaptured rather than reused beside a newer one.
...
- [x] 6a.3 Keep reviewed ceiling machinery absent: capture plans and artifacts contain no ceiling reviews, selector resolution, evidence hashing, protected floor selectors, or per-floor vertical intervals. The walkable-surface cut is task 4.5.
