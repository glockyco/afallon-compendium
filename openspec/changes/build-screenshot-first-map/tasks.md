...
- [ ] 6.5 Verify the representative end-to-end pipeline before bulk collection: one world surface area and one interior through scan, repeat-load identities, optional overworld capture, canonical catalog assembly, and browser picking and details. Include adjacent capture seams, a calibrated interior map on one plane, a producer without a live node, and item-to-source navigation using generated static contracts. Keep incomplete status in publication metadata without adding an interface banner. Acceptance remains pending the full scenario on supported build 25153357.
- [ ] 6.6 Add URL-backed view, selection, query, filter, and global layer state through the canonical parameter set. Verify reload, browser back and forward, travel navigation with its return control, and stale links in the browser. No active-map, singular `layer`, or floor parameter may remain.
...
- [x] 6.9 Give Deck ownership of the live camera, preserve separate captured and orientation-layer snapshots, and keep selection camera-neutral. Reserve persistent full-height control and detail columns. Verify non-inertial pan release, responsive wheel zoom, explicit zoom and fit commands, URL restoration, unchanged map dimensions when details open, and no layer rebuild during view changes in the browser.
...
- [x] 8.9 Allow the trusted `glockyco.com` portfolio to embed the atlas without permitting arbitrary framing.
