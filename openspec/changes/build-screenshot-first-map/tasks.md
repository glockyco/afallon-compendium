...
- [ ] 4.5 Support an optional reviewed clip height in the capture plan, with no clipping by default and no inference. Verify an outdoor extent captures with the plan's own camera frame and a reviewed height reveals covered dungeon content on one plane. No scene object may be disabled, deleted, or left modified.
...
- [x] 6a.2 Remove floor membership end to end: reviewed profile floor domains, floor resolution in spatial extraction and normalization, floor columns and scoping in the database and publication, and floor identity in published contracts. Verify stacked placements keep distinct identities and their own heights, and that no artifact retains a floor field.
- [x] 6a.3 Remove the reviewed ceiling machinery: capture-plan ceiling reviews, selector resolution, evidence hashing, protected floor selectors, and per-floor vertical intervals. Verify no capture input or artifact retains a ceiling review. The conditional clip height is task 4.5.
- [ ] 6a.4 Remove the map and layer selectors from the client once the world layout in 6a.8 lands, so one world map remains navigable. Floor state and its selector are already removed. Verify no stale layer state survives a reload.
