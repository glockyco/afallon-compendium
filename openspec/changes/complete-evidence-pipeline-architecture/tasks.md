...
- [x] 9.5 Enforce a 64 KiB root, 512 KiB atlas/search/geometry parts, and a 3,300,000-byte essential startup total on the frozen representative preview. Verify semantic parity and report excluded search/detail/image/code totals separately rather than hiding transfer cost.
...
- [x] 10.5 Update renderer inputs through stable geometry and semantic layer changes rather than whole-geometry signatures. Verify hover/filter/selection preserve camera inertia and unchanged imagery in the actual browser.
