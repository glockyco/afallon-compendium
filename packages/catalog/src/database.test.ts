...
test("stores every typed fact kind and shared artwork idempotently", () => {
  ...
  const entities = [entity("items", 1), entity("items", 2), entity("npcs", 3), entity("quests", 4), entity("scenes", 5), entity("properties", 6), entity("abilities", 7), entity("recipes", 8), entity("stats", 9), entity("factions", 10), entity("craftingStations", 11), entity("skills", 12)];
  const input: NormalizedDatabaseInput = {
  ...
  try {
    ...
    expect(queryCatalogFacts(db).records.items[0]).toMatchObject({ randomStats: [{ stat: { entityKey: "stats:9" }, min: 1, max: 4, whole: true, chance: 50 }], sockets: [{ socketType: null, gemType: "RED" }], gem: { gemType: "RED", stats: [{ stat: { entityKey: "stats:9" }, amount: 3, isPercent: true }] } });
    for (const table of ["item_facts", "item_stats", "item_random_stats", "item_gem_stats", "item_sockets", "npc_facts", "npc_stats", "npc_ability_phases", "npc_phase_abilities", "npc_faction_rewards", "quest_facts", "quest_objectives", "quest_rewards", "place_facts", "property_facts", "ability_facts", "recipe_facts", "recipe_ranks", "recipe_products", "recipe_materials", "crafting_station_facts"]) expect(db.query(`SELECT count(*) AS count FROM ${table}`).get()).toEqual({ count: 1 });
