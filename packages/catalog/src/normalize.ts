...
import { decodeAbilityGameplay, decodeCraftingStationGameplay, decodedPrice, decodeGearSetGameplay, decodeItemGameplay, decodeNpcGameplay, decodePropertyGameplay, decodeQuestGameplay, decodeRecipeGameplay, decodeRegionGameplay, decodeSceneGameplay, decodeTaskGameplay, validateSupportedSemantics, type GameplayCoverageIssue } from "./decoders";
...
export function collectTypedFacts(admitted: AdmittedCatalog, entities: NormalizedEntity[], bindings: NormalizedDatabaseInput["bindings"], conditions: NormalizedDatabaseInput["conditions"], blockers: Blocker[]): FactRows {
  ...
  for (const [index, item] of admitted.canonical.value.items.entries()) {
    ...
    const itemType = enumName(value.itemType), hasArmorFields = itemType === "ARMOR" || itemType === "Trinket", hasWeaponFields = itemType === "WEAPON";
    ...
    const actions = value.actionAbilities.flatMap((row, actionIndex) => {
      const ability = reference("abilities", row.abilityId, `Ability ${row.abilityId}`, `${path}/actionAbilities/${actionIndex}/abilityId`, provenance);
      return ability === null ? [] : [{ ability, rankIndex: row.rankIndex, sourceIndex: row.sourceIndex }];
    });
    const useLines = value.nativeUseTooltip.text === null ? [] : parseTooltipMarkup(value.nativeUseTooltip.text);
    ...
    rows.itemFacts.push({ entityKey: entityKey("items", item.nativeId), rarity: enumName(value.rarity), itemType, armorSlot: hasArmorFields ? enumName(value.armorSlot) : null, weaponSlot: hasWeaponFields ? enumName(value.weaponSlot) : null, weaponType: hasWeaponFields ? enumName(value.weaponType) : null, armorType: hasArmorFields ? enumName(value.armorType) : null, attackSpeed: value.attackSpeed ?? null, minDamage: value.minDamage ?? null, maxDamage: value.maxDamage ?? null, randomStatsMax: value.randomStatsMax ?? 0, gemType, enchantment, sellPrice, sellCurrency, buyPrice, buyCurrency, stackLimit: value.stackLimit ?? 1, questDropOnly: value.questDropOnly ?? false, corruptionToken: value.isCorruptionToken ?? false, levelRequirement: itemLevels.get(item.nativeId) ?? null, actionAbilities: actions, useLines, conditionIds: conditionIds.get(entityKey("items", item.nativeId)) ?? [], provenance });
  ...
  for (const [index, npc] of admitted.canonical.value.npcs.entries()) {
    ...
    const authoredAdventurer = value.adventurer?.authored === true ? value.adventurer : null;
    const adventurerSpecialization = authoredAdventurer?.specialization.available === true ? authoredAdventurer.specialization : null;
    const adventurer = authoredAdventurer === null ? null : {
      class: reference("classes", authoredAdventurer.classId, `Class ${authoredAdventurer.classId}`, `${path}/adventurer/classId`, provenance),
      race: reference("races", authoredAdventurer.raceId, `Race ${authoredAdventurer.raceId}`, `${path}/adventurer/raceId`, provenance),
      preferredTree: reference("talentTrees", authoredAdventurer.preferredTreeId, `Talent tree ${authoredAdventurer.preferredTreeId}`, `${path}/adventurer/preferredTreeId`, provenance),
      keepPhaseAbilities: authoredAdventurer.keepPhaseAbilities,
      aiLogicTemplateKey: authoredAdventurer.aiLogicTemplateKey,
      specialization: adventurerSpecialization === null ? null : {
        class: reference("classes", adventurerSpecialization.classId, `Class ${adventurerSpecialization.classId}`, `${path}/adventurer/specialization/classId`, provenance),
        role: adventurerSpecialization.role.name,
        preferredTree: reference("talentTrees", adventurerSpecialization.preferredTreeId, `Talent tree ${adventurerSpecialization.preferredTreeId}`, `${path}/adventurer/specialization/preferredTreeId`, provenance),
        behaviorName: adventurerSpecialization.behaviorName,
        priorityAbilities: adventurerSpecialization.priorityAbilities.map((abilityId, abilityIndex) => reference("abilities", abilityId, `Ability ${abilityId}`, `${path}/adventurer/specialization/priorityAbilities/${abilityIndex}`, provenance)).filter((row): row is NormalizedReference => row !== null),
        blockedAbilities: adventurerSpecialization.blockedAbilities.map((abilityId, abilityIndex) => reference("abilities", abilityId, `Ability ${abilityId}`, `${path}/adventurer/specialization/blockedAbilities/${abilityIndex}`, provenance)).filter((row): row is NormalizedReference => row !== null),
        blockedBonuses: adventurerSpecialization.blockedBonuses,
        allowedForms: adventurerSpecialization.allowedForms,
      },
    };
    ...
    for (const [phaseIndex, phase] of (value.aiPhases ?? []).entries()) {
      rows.npcAbilityPhases.push({ entityKey: entityKey("npcs", npc.nativeId), phaseIndex: phase.phaseIndex, name: phase.name || null, requirement: phase.requirement || null, provenance });
      for (const [abilityIndex, contextual] of phase.abilityRefs.entries()) {
        const ability = reference("abilities", contextual.abilityId, `Ability ${contextual.abilityId}`, `${path}/aiPhases/${phaseIndex}/abilityRefs/${abilityIndex}/abilityId`, provenance);
        if (ability) rows.npcPhaseAbilities.push({ entityKey: entityKey("npcs", npc.nativeId), phaseIndex: phase.phaseIndex, abilityIndex, sourceIndex: contextual.sourceIndex, ability, rankIndex: contextual.rankIndex, provenance });
      }
    }
  ...
  for (const family of supportFamilies) for (const [index, support] of (admitted.support.value.tables[family] ?? []).entries()) {
    ...
    if (family === "abilities") {
      ...
      const value = decodeAbilityGameplay(support.gameplay, admitted.support.reference, `${path}/gameplay`).value;
      rows.abilityFacts.push({
        entityKey: key,
        ranks: value.ranks.map((rank, rankIndex) => ({ rankIndex: rank.rankIndex, lines: parseTooltipMarkup(rank.text), provenance: [pointer(admitted.support.reference, `${path}/gameplay/ranks/${rankIndex}`)] })),
        provenance,
      });
    }
    ...
    else if (family === "gearSets" && support.gameplay !== undefined) {
      const value = decodeGearSetGameplay(support.gameplay, admitted.support.reference, `${path}/gameplay`).value;
      rows.gearSetFacts.push({ entityKey: key, provenance });
      for (const [memberIndex, member] of value.itemsInSet.entries()) { const item = reference("items", member.itemId, `Item ${member.itemId}`, `${path}/gameplay/itemsInSet/${memberIndex}`, provenance); if (item) rows.gearSetMembers.push({ entityKey: key, memberIndex, item, provenance }); }
      for (const tier of value.gearSetTiers) {
        rows.gearSetTiers.push({ entityKey: key, tierIndex: tier.tierIndex, equipped: tier.equippedAmount, provenance });
        for (const [statIndex, tierStat] of tier.stats.entries()) { const statRef = reference("stats", tierStat.statId, `Stat ${tierStat.statId}`, `${path}/gameplay/gearSetTiers/${tier.tierIndex}/stats/${statIndex}`, provenance); if (statRef) rows.gearSetTierStats.push({ entityKey: key, tierIndex: tier.tierIndex, statIndex, stat: statRef, amount: tierStat.amount, isPercent: tierStat.isPercent || (statPercent.get(tierStat.statId) ?? false), provenance }); }
      }
    }
  }
...
export function normalizeCatalog(admitted: AdmittedCatalog, planReference: ArtifactReference): NormalizedDatabaseInput {
  ...
  for (const condition of conditions) {
    if (condition.ownerType !== "entity" || !condition.ownerKey.startsWith("items:")) continue;
    const classification = classifyItemCondition(condition.payload);
    condition.scope = classification.scope;
    if (classification.scope === null && classification.requirementTypes.length > 0) blockers.push({ kind: "unclassified-item-requirement", key: `${condition.ownerKey}:${classification.requirementTypes.join(",")}`, detail: `Item condition contains unclassified or mixed requirement types: ${classification.requirementTypes.join(", ")}.`, provenance: condition.provenance });
  }
