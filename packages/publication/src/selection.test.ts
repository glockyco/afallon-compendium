...
import type { StaticCoverage, StaticEntityDetail, StaticEntitySearch, StaticItemSearch, StaticRootManifest } from "@afallon/contracts/public";
import { PUBLICATION_ESSENTIAL_BUDGET, writeStaticJson } from "./resources";
...
test("rejects broken nested edges and damaged reuse without replacing the selected publication", async () => {
  ...
  try {
    ...
    await selectPublication(store, publicationRoot, rootResource, resources, [], gate);

    await expect(selectPublication(store, publicationRoot, rootResource, resources.slice(0, -1), [], gate)).rejects.toThrow("missing");
    await expect(selectPublication(store, publicationRoot, { reference: { ...rootResource.reference, sha256: "f".repeat(64) }, identity: rootResource.identity }, resources, [], gate)).rejects.toThrow("reference");
    await expect(selectPublication(store, publicationRoot, rootResource, resources, [], { ...gate, accepted: false })).rejects.toThrow("catalog gate");
    const wrongRoot = await writeStaticJson(store, rootValue.schemaVersion, { ...rootValue, guides: { overview: entity.reference } });
    await expect(selectPublication(store, publicationRoot, wrongRoot, resources, [], gate)).rejects.toThrow();
    ...
    await expect(selectPublication(store, publicationRoot, unsafeRoot, resources, [], gate)).rejects.toThrow();
    ...
    await expect(selectPublication(store, publicationRoot, wrongBuildRoot, [wrongEntity, item, coverage, wrongDetail], [], gate)).rejects.toThrow("build mismatch");
    ...
    await expect(selectPublication(store, publicationRoot, oversizedRoot, [entity, item, largeCoverage, detail], [], gate)).rejects.toThrow();
    ...
    await expect(selectPublication(store, publicationRoot, rootResource, resources, [], gate)).rejects.toThrow("mismatch");
    expect(await readFile(join(publicationRoot, "selected.json"), "utf8")).toBe(selectedBytes);
