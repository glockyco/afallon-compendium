...
test("selects a complete candidate atomically and preserves it after validation failures", async () => {
  ...
  try {
    ...
    const rootValue: StaticRootManifest = { schemaVersion: "compendium.static-root.v1", ...identity, mode: "preview", complete: false, world: { mapSpaceId: "world", label: "Afallon", bounds: { min: { x: 0, y: 0 }, max: { x: 1, y: 1 } }, offsets: [{ mapSpaceId: "world", worldX: 0, worldY: 0, source: "native", status: "placed" }], unplacedMapSpaceIds: [] }, maps: [], entitySearch: entity.reference, itemSearch: item.reference, guides: { overview: entity.reference }, coverage: coverage.reference };
