...
type RetainInput = (path: string, expected: { sha256: string; bytes: number }) => Promise<void>;
...
async function verifyIndexFiles(config: CompendiumConfig, index: SerializedAssetIndex, label: string, runtime: Runtime, retainInput: RetainInput): Promise<void> {
  ...
  for (const file of files) {
    ...
    await retainInput(canonicalPath, { sha256: file.sha256, bytes: file.bytes });
    checked.set(canonicalPath, file.sha256);
    checkSignal(runtime);
  }
}
...
export interface PlacementIdentityPreparation {
  ...
  indexes: Array<{ path: string; value: SerializedAssetIndex }>;
  issues: SceneSourceIssues;
}
...
export async function prepareSceneIdentities(
  ...
  await verifyIndexFiles(config, sceneIndex, "Scene serialized asset index", runtime, retainInput);
  checkSignal(runtime);
  ...
  const indexes = [{ path: scenePath, value: sceneIndex }];
  ...
  for (const [guid, prepared] of [...sourceByGuid.entries()].sort(([left], [right]) => left.localeCompare(right))) {
    ...
    await verifyIndexFiles(config, index, `Prefab serialized asset index ${guid}`, runtime, retainInput);
    checkSignal(runtime);
    ...
    indexes.push({ path: prefabPath, value: index });
  }
  ...
  checkSignal(runtime);
  return { result, artifactPaths: [scenePath, ...prefabPaths, issuesPath, resultPath], indexes, issues: issueArtifact };
}
